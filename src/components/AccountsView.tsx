...
export const AccountsView: React.FC<AccountsViewProps> = ({
  ...
  const handleStartOAuth = async () => {
    ...
    try {
      ...
      console.warn('OAuth start notice:', err);
      handleConnect();
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4">
          <div className="w-full max-w-md rounded-2xl bg-slate-900 border border-slate-700 p-6 space-y-4 text-slate-100 shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              ...
              {/* One-click Google Sign-In button when provider is Gmail */}
              {newProvider === 'gmail' && (
                <div className="space-y-2">
                  <button
                    id="connect-google-oauth-modal-btn"
                    type="button"
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-xs font-semibold shadow-xs border transition ${
                      isStartingOAuth
                  ...
                  >
                    ...
                    <span>{isStartingOAuth ? 'Connecting Google Account...' : 'Authorize & Connect Live Google Account'}</span>
                  </button>
                  <div className="flex items-center gap-2 text-[10px] text-slate-500 justify-center">
                    <div className="h-px bg-slate-800 flex-1" />
                    <span>OR MANUALLY ENTER MAILBOX</span>
                    <div className="h-px bg-slate-800 flex-1" />
                  </div>
                </div>
              )}
